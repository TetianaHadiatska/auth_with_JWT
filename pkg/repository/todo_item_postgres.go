...
func (r *TodoItemPostgres) Create(listId int, item auth.TodoItem) (int, error) {
	tx, err := r.db.Begin()
	if err != nil {
		return 0, err
	}
...
func (r *TodoItemPostgres) GetAll(userId, listId int) ([]auth.TodoItem, error) {
	var items []auth.TodoItem
...
func (r *TodoItemPostgres) GetById(userId, itemId int) (auth.TodoItem, error) {
	var item auth.TodoItem
...
func (r *TodoItemPostgres) Update(userId, itemId int, input auth.UpdateItemInput) error {
